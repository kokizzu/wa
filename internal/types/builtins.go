...
func (check *Checker) builtin(x *operand, call *ast.CallExpr, id builtinId) (_ bool) {
	...
	case _Append:
		...
		if nargs == 2 && call.Ellipsis.IsValid() && x.assignableTo(check, NewSlice(check._universeByte()), nil) {
			...
			if isString(x.typ) {
				if check.Types != nil {
					sig := makeSig(check.pkg.W2Mode, S, S, x.typ)
					sig.variadic = true
		...
		sig := makeSig(check.pkg.W2Mode, S, S, NewSlice(T)) // []T required for variadic signature
		sig.variadic = true
	...
	case _Cap, _Len:
		...
		if check.Types != nil && mode != constant_ {
			check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, x.typ, typ))
		}
	...
	case _Complex:
		...
		if check.Types != nil && x.mode != constant_ {
			check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, resTyp, x.typ, x.typ))
		}
	...
	case _Copy:
		...
		if check.Types != nil {
			check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, Typ[Int], x.typ, y.typ))
		}
		x.mode = value
		x.typ = Typ[Int]
	...
	case _Delete:
		...
		x.mode = novalue
		if check.Types != nil {
			check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, nil, m, m.key))
		}
	...
	case _Imag, _Real:
		...
		if check.Types != nil && x.mode != constant_ {
			check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, resTyp, x.typ))
		}
	...
	case _Make:
		...
		if check.Types != nil {
			...
			check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, x.typ, params[:1+len(sizes)]...))
		}
	...
	case _New:
		...
		var sig *Signature
		if nargs == 2 {
			sig = makeSig(check.pkg.W2Mode, x.typ, T, T)
			...
			sig = makeSig(check.pkg.W2Mode, x.typ, T)
		}
	...
	case _Panic:
		...
		x.mode = novalue
		if check.Types != nil {
			check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, nil, &emptyInterface))
		}
	...
	case _Print, _Println:
		...
		x.mode = novalue
		if check.Types != nil {
			check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, nil, params...))
		}
	...
	case _unsafe_Raw:
		...
		if check.Types != nil {
			check.recordBuiltinType(
				call.Fun, makeSig(check.pkg.W2Mode, retType, x.typ),
			)
		}
	...
	case _Assert:
		...
		if strings.HasSuffix(filename, ".wa") {
			...
			x.mode = novalue
			if check.Types != nil {
				check.recordBuiltinType(call.Fun, makeSig(check.pkg.W2Mode, nil, x.typ))
			}
